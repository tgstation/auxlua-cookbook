...
local invisibleHolder = SS13.new("/obj", dm.global_proc("_locate", 169, 71, 1))

invisibleHolder:set_var("resistance_flags", 243)
sleep()
local lighter = SS13.new("/obj/structure", dropLocation)
local iconToSteal = SS13.new("/obj/machinery/telecomms/relay")

lighter:set_var("icon", iconToSteal:get_var("icon"))
lighter:set_var("icon_state", iconToSteal:get_var("icon_state"))
lighter:set_var("name", "Bluespace Room")
lighter:set_var("w_class", 6)
lighter:set_var("desc", "Contains a label on the side: <span class='notice'>Touch to use</span>.")
lighter:set_var("resistance_flags", 243)
lighter:set_var("anchored", 0)
lighter:set_var("density", 0)

local processingPlayers = {}
local antiPeople = {}

local teleportDestination = SS13.new("/obj/structure", dropLocation)

teleportDestination:set_var("icon_state", "floorsafe")
teleportDestination:set_var("name", "Exit")
teleportDestination:set_var("w_class", 6)
teleportDestination:set_var("desc", "Contains a label on the side: <span class='notice'>Touch to use</span>.")
teleportDestination:set_var("resistance_flags", 243)
teleportDestination:set_var("anchored", 1)
teleportDestination:set_var("density", 0)

dm.global_proc("qdel", iconToSteal)
...
SS13.register_signal(teleportDestination, "atom_attack_hand", function(self, player)
    local playerRef = dm.global_proc("REF", player)
...
SS13.register_signal(lighter, "movable_moved", function(self)
    ...
    for index, universeTurf in ipairs(turfs) do
        ...
        if universeTurf:get_var("icon") == nil then
            ...
            universeTurf:set_var("appearance", turf:get_var("appearance"))
        end
